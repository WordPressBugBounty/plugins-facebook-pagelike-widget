...
use BZSocialFeeds\Admin\Admin;
use BZSocialFeeds\Utils\Settings;
use BZSocialFeeds\Api\Api;
use BZSocialFeeds\Utils\PermissionCheck;

/**
 * Get current languages
 */
function bzsfGetCurrentLanguage()
{
    // Polylang
 ...
 */
function bzsf_redirect_to_page()
{
    // Validate params
    if (!isset($_GET['page_id']) || !is_numeric($_GET['page_id']) || !isset($_GET['is_bz_social_feeds_redirect'])) {
        return;
    }
 ...
 */
add_action('template_redirect', 'bzsf_redirect_to_page', 0);
...
add_action('wp_head', function () {
    if (Settings::getSetting("site_id")) {
        ...
        $pageData = [
            "language" => bzsfGetCurrentLanguage()
        ];
...
function bzsfWidgetShortcode($atts)
{
    // Get attributes
    $atts = shortcode_atts(
        array(
    ...
    if (!isset($atts['id']) || !is_string($atts['id']) || !bzsfIsValidUUID($atts['id'])) return "";

    return '<div class="buttonizer-inline-widget" data-buttonizer-widget-id="' . $atts['id'] . '"></div>';
};

function bzsfInitFunction()
{
    if (!shortcode_exists("buttonizer")) {
        add_shortcode('buttonizer', 'bzsfWidgetShortcode');
    }
}

add_action('init', 'bzsfInitFunction');
