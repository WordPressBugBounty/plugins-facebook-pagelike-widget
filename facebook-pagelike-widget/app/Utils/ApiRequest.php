...
class ApiRequest
{
    ...
    public static function saveApiToken(string $token)
    {
        ...
        register_setting(BZ_SOCIAL_FEEDS_NAME,  BZ_SOCIAL_FEEDS_NAME . '_site_connection', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field'
        ]);
        update_option(BZ_SOCIAL_FEEDS_NAME . '_site_connection', $token);
    ...
    public static function getApiToken()
    {
        ...
        // Connect Site ID
        $token = get_option(BZ_SOCIAL_FEEDS_NAME . '_site_connection');

        // Token not available. Get from transient instead
        if (!$token) {
            $token = get_transient(BZ_SOCIAL_FEEDS_NAME . '_site_connection');
            ...
            delete_transient(BZ_SOCIAL_FEEDS_NAME . '_site_connection');
        }
    ...
    public static function deleteApiToken()
    {
        ...
        delete_option(BZ_SOCIAL_FEEDS_NAME . '_site_connection');
    ...
    public static function post(string $path = '/', array $body = [], array $headers = [], $withAuthorization = true, $refreshtokenCheck = true)
    {
        ...
        $args = [
            'body' => $body,
            'headers' => $headers,
            "timeout" => defined("BZ_SOCIAL_FEEDS_API_TIMEOUT") ? BZ_SOCIAL_FEEDS_API_TIMEOUT : 20
        ];

        // Execute
        $response = wp_remote_post(BZ_SOCIAL_FEEDS_API_URI . ($withAuthorization ? '/external' : '') . $path, $args);
